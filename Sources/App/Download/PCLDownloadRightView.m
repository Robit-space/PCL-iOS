...
- (void)updateVersionTableHeight{NSInteger n=[self numberOfSectionsInTableView:self.versionTableView];CGFloat h=0;if(n==1)h=[self ce:self.filteredVersions.count*44];else{h=[self ce:63+MIN(2,[self versionsForSection:0].count)*42];for(NSInteger i=1;i<n;i++)h+=[self ce:44+([self.expandedSections containsObject:@(i)]?[self versionsForSection:i].count*44+30:14)];}for(NSLayoutConstraint*c in self.versionTableView.constraints)if(c.firstAttribute==NSLayoutAttributeHeight){c.constant=MAX(1,h);break;}}
...
- (UIView*)latestHeader:(CGFloat)w{
 NSArray*a=[self versionsForSection:0];CGFloat h=[self ce:48+MIN(2,a.count)*42];UIView*wra=[[UIView alloc]initWithFrame:CGRectMake(0,0,w,h+[self ce:15])];wra.tag=820;UIView*c=[[UIView alloc]initWithFrame:CGRectMake(0,0,w,h)];c.backgroundColor=[UIColor colorWithWhite:.995 alpha:.94];c.layer.cornerRadius=[self ce:11];[wra addSubview:c];
 ...
 for(NSInteger i=0;i<MIN(2,a.count);i++){NSDictionary*d=a[i];NSString*y=d[@"type"]?:@"",*z=d[@"releaseTime"]?:@"";PCLDownloadVersionCell*r=[[PCLDownloadVersionCell alloc]initWithStyle:0 reuseIdentifier:nil];r.frame=CGRectMake(20,[self ce:40+i*42],w-38,[self ce:42]);r.tag=i;r.contentView.backgroundColor=UIColor.clearColor;r.versionIcon.image=[UIImage imageNamed:[y isEqualToString:@"release"]?@"CEGrass":@"CECommandBlock"];r.versionIcon.transform=CGAffineTransformMakeScale(1.1,1.1);r.versionLabel.font=[UIFont systemFontOfSize:14 weight:UIFontWeightMedium];r.dateLabel.font=[UIFont systemFontOfSize:13];r.versionLabel.text=d[@"id"]?:@"";NSString*day=z.length>10?[z substringToIndex:10]:z;r.dateLabel.text=[NSString stringWithFormat:@"%@，发布于 %@",[y isEqualToString:@"release"]?@"最新正式版":@"最新预览版",day];[r addGestureRecognizer:[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(latestTapped:)]];[c addSubview:r];}return wra;
}
...
- (CGFloat)tableView:(UITableView*)t heightForHeaderInSection:(NSInteger)n{return [self numberOfSectionsInTableView:t]>1?[self ce:n?44:63+MIN(2,[self versionsForSection:0].count)*42]:.01;}
