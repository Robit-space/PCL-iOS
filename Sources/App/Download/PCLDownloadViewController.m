...
- (void)animateLeftBackgroundFrom:(CGFloat)a to:(CGFloat)b{UIView*v=[self.view viewWithTag:777];self.animatingLeftBackground=YES;CGRect f=v.frame;f.size.width=a;v.frame=f;f=self.shadowView.frame;f.origin.x=a;self.shadowView.frame=f;[UIView animateWithDuration:.10 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{CGRect x=v.frame;x.size.width=b;v.frame=x;x=self.shadowView.frame;x.origin.x=b;self.shadowView.frame=x;} completion:^(BOOL d){self.animatingLeftBackground=NO;}];}
