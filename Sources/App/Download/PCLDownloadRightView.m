...
- (UIView*)categoryHeader:(NSInteger)n width:(CGFloat)w{
 ...
 UIButton*q=[[UIButton alloc]initWithFrame:v.bounds];q.tag=n;BOOL o=[self.expandedSections containsObject:@(n)];[q setImage:[UIImage systemImageNamed:o?@"arrowtriangle.down.fill":@"arrowtriangle.right.fill"] forState:0];q.imageView.preferredSymbolConfiguration=[UIImageSymbolConfiguration configurationWithPointSize:8 weight:UIImageSymbolWeightBold];q.tintColor=PCLColor(0x505A66);q.contentHorizontalAlignment=UIControlContentHorizontalAlignmentRight;q.contentEdgeInsets=UIEdgeInsetsMake(0,0,0,16);[q addTarget:self action:@selector(toggleVersionSection:) forControlEvents:UIControlEventTouchUpInside];[v addSubview:q];return v;
}
